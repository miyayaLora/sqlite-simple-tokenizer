...
> 这是一个使用 `rusqlite` 构建 SQLite fts5 插件的项目，其主要功能是为 SQLite 提供多语言分词。这个项目可以作为 Rust 的 crate 使用，也可以将其编译成动态库在 SQLite 中加载和使用。

## 简介

这个项目提供一种 SQLite 分词器 `charabia_tokenizer`，可处理多种语言分词
...
- 在 `sqlite` 中使用 `.load libsqlite_charabia_tokenizer` 进行加载
...
(
    text,
    tokenize = 'charabia disable_chinese_stopword'
);
...
在 Rust 中使用这个分词器，需要引入 `rusqlite` 依赖， 使用 `cargo add rusqlite sqlite-charabia-tokenizer` 安装依赖
...
// 创建一个测试表
conn.execute(
    "CREATE VIRTUAL TABLE t1 USING fts5(text, tokenize = 'charabia');",
    [],
).unwrap();
// 插入数据
conn.execute(
    r#"INSERT INTO t1(text) VALUES ('中华人民共和国国歌'),('静夜思'),('国家'),('举头望明月');"#,
    [],
).unwrap();
conn.execute(r#"INSERT INTO t1(text) VALUES ('Thé quick brown');"#, [])
    .unwrap();
let mut stmt = conn
...
let mut stmt = conn
    .prepare("SELECT * FROM t1 WHERE text MATCH 'The';")
    .unwrap();
let result = stmt
    .query_map([], |row| Ok(row.get::<_, String>(0).unwrap()))
    .unwrap();
...
assert_eq!(["Thé quick brown"], vec.as_slice());
```
