...
#[unsafe(no_mangle)]
pub extern "C" fn sqlite3_sqlitecharabiatokenizer_init(
    db: *mut sqlite3,
...
fn init(db: Connection) -> rusqlite::Result<bool> {
    ...
    match crate::load(&db) {
        Ok(()) => {
            info!("[sqlite-charabia-tokenizer] initialized");
            Ok(false)
        }
        Err(error) => {
            error!("[sqlite-charabia-tokenizer] initialization failed: {error:?}");
